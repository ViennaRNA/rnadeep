...
def spotrna(model=0,use_mask = False):
    ...
    if (num_lstm_blocks>0):
        ...
        #LSTM blocks
        t = diagonal_lstm_block(t, neurons=lstm_neurons, third_dim=block_num_filters)
        for j in range(num_lstm_blocks-1):
            t = diagonal_lstm_block(t, neurons=lstm_neurons,third_dim= lstm_neurons*4)
...
def main():
    m = spotrna(1,True)
    directory = "/scr/romulus/jwielach/rnadeepfold/data/processed/uniform/25_100"
    masks = np.load(os.path.join(directory,"train" ,"matrices.npy"),mmap_mode="r")
    masks = np.take(masks, range(0,1000), axis=0)
    ytrain = np.load(os.path.join(directory,"train" ,"padded_structures.npy"),mmap_mode="r")
    ytrain = np.take(ytrain , range(0,1000), axis=0)
    xtrain = np.load(os.path.join(directory, "train","padded_sequences.npy"),mmap_mode="r")
    xtrain = np.take(xtrain, range(0,1000), axis=0)
    ...
    #csvname = os.path.join("%s_%s.csv" % (filename,loss))
    #csv_logger = CSVLogger(csvname, append=True, separator=';')
    m.compile(optimizer="adam",loss=loss, metrics=['acc',mcc]) #,sensitivity,specificity,f1])
    #m.fit(xtrain, ytrain, batch_size=32,epochs=10,shuffle = True,verbose=1,validation_data=(xval,yval))
    m.fit([xtrain,masks], ytrain, batch_size=64,epochs=10,shuffle = True,verbose=1) #,callbacks=[csv_logger])
    #predictions = m.predict(xval)
    #print((np.abs(predictions) < 0.5).all())
    #m.save("./%s_%s" % (filename,loss))
